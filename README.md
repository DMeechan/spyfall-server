...
```
git clone https://github.com/dmeechan/spyfall-server.git
```
...
1. Open up the project directory in *two* Terminal windows
2. In the first Terminal window, enter: `yarn watch-ts`
3. In the second Terminal window, enter: `yarn watch-node`
4. Now open up [localhost:3000](http://localhost:3000)

The first command will watch for TypeScript code changes in `src` and automatically transpile them into JavaScript and place them in the `dist` folder.

The second command will start your Node.js server then watch for changes to files in the `dist` folder. If any files change, the server will be automatically reloaded (very handy for develoment!).
